...
//Need these:
//extern int stuber_open_r(struct _reent *, const char *, int, int);
//extern int stuber_close_r(struct _reent *, int);
//extern int stuber_read_r(struct _reent *, int, void *, size_t);
//extern int stuber_write_r(struct _reent *, int, const void *, size_t);
//extern int stuber_lseek_r(struct _reent *, int, int, int);
//extern int stuber_fstat_r(struct _reent *, int, void *);
//extern int stuber_isatty_r(struct _reent *, int);
//extern int stuber_stat_r(struct _reent *, const char *, void *);
//extern int stuber_gettimeofday_r(struct _reent *r, void *, void *);
//extern int stuber_unlink_r(struct _reent *r, const char *path);
...
int stuber_stat_r(struct _reent *r, const char *path, struct stat *st) {
    char* f_path = (char*)path; //format_path((char*)path);
...
int stuber_fstat_r(struct _reent *r, int fd, struct stat *st) {
    return _stat_r(r,fd_paths[fd], st);
}


int stuber_isatty_r(struct _reent *r, int fd) { 
    return (fd >= 0 && fd <= 2) ? 1 : 0;  // stdin, stdout, stderr, uart are ttys
}

// _lseek_r
off_t stuber_lseek_r(struct _reent *r, int fd, off_t offset, int whence) {
...
int stuber_unlink_r(struct _reent *r, const char *path) {
    char* f_path = (char*)path;//format_path((char*)path);
...
int stuber_gettimeofday_r(struct _reent *r, struct timeval *tp, void *tzp) 
{ 
...
_ssize_t stuber_write_r(struct _reent *r, int fd, const void *buf, size_t count) {
    uint16_t *wbuf;
    size_t cnt = count;
    switch (fd) {
    case STDOUT_FILENO:
...
_ssize_t stuber_read_r(struct _reent *r, int fd, void *buf, size_t count) {
    size_t i;
    long result;
    char* bufc = buf;
    ...
    default:
        ...
        if (fd >= STD_FD_COUNT && fd < MAX_OPEN_FILES && fd_table[fd]) {
            ...
            if ((r->_errno = ext4_fread(fd_table[fd], bufc, count, &br)))
                return -1;
...
int stuber_open_r(struct _reent *r, const char *path, int flags, int mode) {
...
int stuber_close_r(struct _reent *r, int fd) {
    if (fd < STD_FD_COUNT || fd >= MAX_OPEN_FILES || !fd_table[fd]) {
        r->_errno = EBADF;
        return -1;
    }
