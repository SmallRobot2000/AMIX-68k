...
//Need these:
//extern int _stub_open_r(struct _reent *, const char *, int, int);
//extern int _stub_close_r(struct _reent *, int);
//extern int _stub_read_r(struct _reent *, int, void *, size_t);
//extern int _stub_write_r(struct _reent *, int, const void *, size_t);
//extern int _stub_lseek_r(struct _reent *, int, int, int);
//extern int _stub_fstat_r(struct _reent *, int, void *);
//extern int _stub_isatty_r(struct _reent *, int);
//extern int _stub_stat_r(struct _reent *, const char *, void *);
//extern int _stub_gettimeofday_r(struct _reent *r, void *, void *);
//extern int _stub_unlink_r(struct _reent *r, const char *path);
...
int _stub_stat_r(struct _reent *r, const char *path, struct stat *st) {
...
int _stub_fstat_r(struct _reent *r, int fd, struct stat *st) {
    return _stat_r(r,fd_paths[fd], st);
}


int _stub_isatty_r(struct _reent *r, int fd) { 
    return (fd >= 0 && fd <= 2) ? 1 : 0;  // stdin, stdout, stderr, uart are ttys
}

// _lseek_r
off_t _stub_lseek_r(struct _reent *r, int fd, off_t offset, int whence) {
...
int _stub_unlink_r(struct _reent *r, const char *path) {
    char* f_path = (char*)path;//format_path((char*)path);
...
int _stub_gettimeofday_r(struct _reent *r, struct timeval *tp, void *tzp) 
{ 
...
_ssize_t _stub_write_r(struct _reent *r, int fd, const void *buf, size_t count) {
    uint16_t *wbuf;
    size_t cnt = count;
    switch (fd) {
    case STDOUT_FILENO:
...
_ssize_t _stub_read_r(struct _reent *r, int fd, void *buf, size_t count) {
    size_t i;
    long result;
    char* bufc = buf;
...
int _stub_open_r(struct _reent *r, const char *path, int flags, int mode) {
...
int _stub_close_r(struct _reent *r, int fd) {
    if (fd < STD_FD_COUNT || fd >= MAX_OPEN_FILES || !fd_table[fd]) {
        r->_errno = EBADF;
        return -1;
    }
